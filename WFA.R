...
add.distribution(strat.st,
  ...
  variable = list(n = seq(10,20,by=5)),
  label = 'n'
)
...
if( !file.exists("saveEnvironment.RData") )
{
  ...
  save(list=c("res",".strategy",".blotter"),file="saveEnvironment.RData")
} else {
...
# colnames(.audit$portfolio.opt.9$summary)
## ----TRAIN20082011-------------------------------------------------------
chart.forward.training("wfa.USO.2008-01-02.2011-12-30.RData")
load("wfa.USO.2008-01-02.2011-12-30.RData")
head(.audit$order_book.opt.1$opt.1$USO)
save(.audit, file ="wfa.GILD_30MIN.2015-06-01-063000.2015-09-30-123000.RData" )
head(as.Date(index(GILD_30MIN)))

load('wfa.GILD_30MIN.2015-08-03.2015-11-30.RData')
.audit$
p <- getPortfolio(portf.st, envir = .audit)
from <- index(p$summary[2])
R <- cumsum(p$summary[paste(from, "/", sep = ""), "Net.Trading.PL"])


# plot(index(.audit$portfolio.opt.5$summary),cumsum(.audit$portfolio.opt.5$summary[,9]),type="l")
## ----TRAIN20092012-------------------------------------------------------
chart.forward.training("c:/users/tyamada/Google Drive/CFRM551/wfa.USO.2009-01-02.2012-12-31.RData")
chart.forward.training('wfa.GILD_30MIN.2015-10-01.2016-01-29.RData')
chart.forward.training('wfa.GILD_30MIN.2015-08-03.2015-11-30.RData')
chart.forward.training2('wfa.USO.2009-01-02.2012-12-31.RData')
chart.forward.training('wfa.USO.2007-01-03.2007-08-31.RData')
load('wfa.USO.2007-01-03.2007-08-31.RData')
ls(.audit)
load('wfa.GILD_30MIN.2015-08-03.2015-11-30.RData')
ls(.audit)
## ----TESTOPT-------------------------------------------------------------
